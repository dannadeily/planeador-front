...
import CrearSubtemaExcel from "./CrearSubtemaExcel";
    ...
    const fetchData = async () => {
      try {
        ...
        if (response.data.length > 0) {
          setUnidad_tematica_id(response.data[0].id); // Establecer la primera categoría como seleccionada por defecto
        }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.status === 200) {
        ...
        setNombre("");
        setDescripcion("");
        if (unidad_tematica.length > 0) {
          setUnidad_tematica_id(categorias[0].id); // Restablecer a la primera categoría
        }
      }
    } catch (error) {
